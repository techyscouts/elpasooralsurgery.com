@if (isset($content->hide_section) && $content->hide_section == 'no')
    <section class="content-with-button lg:pb-100 pb-50 lg:pt-100 pt-50 @if ($content->extra_class) {!! $content->extra_class !!} @endif"
                    ...
                    <div class="btn-custom btn-inline gap-5 lg:gap-10 justify-center">
                        @foreach ($content->buttons as $button)
                            @if (!empty($button['button']))
                                <a href="{!! $button['button']['url'] !!}" @if ($button['button']['target']) target="{!! $button['button']['target'] !!}" @else target="_self" @endif class="btn btn-border-grey min-w-[200px]">{!! $button['button']['title'] !!}</a>
                            @endif
                        @endforeach
                    </div>
                @endif
